...
auto.ortho <- auto.dum[, -1] %>% 
  ...
  as.matrix() %>% 
  orthonormalization(basis = FALSE, norm = FALSE)
...
```{r}
plot(sort(Hitters$Salary))
```

Set threshold as 250. 

```{r}
hitters.class <- Hitters %>% 
  filter(!is.na(Salary)) %>% 
...
```{r}
# hitters.pred <- hitters.class %>% 
#   mutate(obbs = hitters.logistic$fitted.values, 
#          obbs = exp(obbs), 
#          p = obbs / (1 + obbs), 
#          Salary_p = if_else(p > 0.5, 1, 0))

hitters.pred <- hitters.class %>% 
  mutate(Salary_p = round(hitters.lr$fitted.values), 
         acc = if_else(Salary == Salary_p, 1, 0))

sum(hitters.pred$acc) / nrow(hitters.pred)
```

Kernel density estimation. 

```{r}
hitters.error <- hitters.pred %>% 
  filter(acc == 0)
```
